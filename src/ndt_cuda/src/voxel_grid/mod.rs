...
mod tests {
    ...
    fn test_mahalanobis_distance_stats() {
        ...
        crate::test_println!(
            ...
            _total_correspondences,
            _total_correspondences as f64 / n as f64
        );
    ...
    /// Compare actual voxel dumps from CUDA and Autoware to identify covariance discrepancy source.
    ///
    /// This test loads the JSON dump files and compares matching voxels to understand
    /// why CUDA covariances are ~66% of Autoware's despite correct formula.
    ///
    /// Run with: cargo test -p ndt_cuda --lib "test_compare_voxel_dumps" --features test-verbose -- --nocapture
    #[test]
    fn test_compare_voxel_dumps() {
        use std::collections::HashMap;
        use std::fs::File;

        // Paths to voxel dumps (generated by NDT_DUMP_VOXELS=1)
        let cuda_path = "/home/aeon/repos/cuda_ndt_matcher/logs/ndt_cuda_voxels.json";
        let autoware_path = "/home/aeon/repos/cuda_ndt_matcher/logs/ndt_autoware_voxels.json";

        // Skip test if files don't exist
        if !std::path::Path::new(cuda_path).exists() {
            crate::test_println!("Skipping: CUDA voxel dump not found at {}", cuda_path);
            return;
        }
        ...
        crate::test_println!("\n=== VOXEL DUMP COMPARISON ===");

        // Load both dumps
        let cuda_file = File::open(cuda_path).expect("Failed to open CUDA dump");
        let cuda_dump: VoxelDumpJson =
            serde_json::from_reader(cuda_file).expect("Failed to parse CUDA dump");

        let autoware_file = File::open(autoware_path).expect("Failed to open Autoware dump");
        ...
        crate::test_println!(
            ...
            autoware_dump.resolution
        );

        // Build spatial index for Autoware voxels (mean -> voxel)
        // Use discretized mean position as key for matching
        let resolution = autoware_dump.resolution;
        let autoware_map: HashMap<(i32, i32, i32), &VoxelJson> = autoware_dump
            .voxels
            .iter()
            .map(|v| {
                let key = (
                    (v.mean[0] / resolution).floor() as i32,
                    (v.mean[1] / resolution).floor() as i32,
                    (v.mean[2] / resolution).floor() as i32,
                );
                (key, v)
            })
            .collect();

        // Match CUDA voxels to Autoware voxels
        let mut matched = 0;
        let mut cov_ratios_by_point_count: HashMap<usize, Vec<f32>> = HashMap::new();
        let mut mean_diffs: Vec<f32> = Vec::new();
        let mut point_count_diffs: Vec<i32> = Vec::new();

        for cuda_voxel in &cuda_dump.voxels {
            let key = (
                (cuda_voxel.mean[0] / resolution).floor() as i32,
                (cuda_voxel.mean[1] / resolution).floor() as i32,
                (cuda_voxel.mean[2] / resolution).floor() as i32,
            );

            if let Some(aw_voxel) = autoware_map.get(&key) {
                matched += 1;

                // Compute mean difference
                let dx = cuda_voxel.mean[0] - aw_voxel.mean[0];
                let dy = cuda_voxel.mean[1] - aw_voxel.mean[1];
                let dz = cuda_voxel.mean[2] - aw_voxel.mean[2];
                let mean_diff = (dx * dx + dy * dy + dz * dz).sqrt();
                mean_diffs.push(mean_diff);

                // Point count difference
                let pc_diff = cuda_voxel.point_count as i32 - aw_voxel.point_count as i32;
                ...
                if aw_trace > 1e-10 {
                    let ratio = cuda_trace / aw_trace;
                    cov_ratios_by_point_count
                        .entry(cuda_voxel.point_count)
                        .or_default()
                        .push(ratio);
                }
            }
        ...
        crate::test_println!("\n--- Matching Results ---");
        crate::test_println!(
            "Matched voxels: {} / {} CUDA voxels ({:.1}%)",
            matched,
            cuda_dump.voxels.len(),
            matched as f32 / cuda_dump.voxels.len() as f32 * 100.0
        );

        // Mean difference statistics
        if !mean_diffs.is_empty() {
            mean_diffs.sort_by(|a, b| a.partial_cmp(b).unwrap());
            let _avg: f32 = mean_diffs.iter().sum::<f32>() / mean_diffs.len() as f32;
            crate::test_println!("\nMean position difference (m):");
            crate::test_println!("  min: {:.6}", mean_diffs.first().unwrap());
            crate::test_println!("  median: {:.6}", mean_diffs[mean_diffs.len() / 2]);
            crate::test_println!("  max: {:.6}", mean_diffs.last().unwrap());
            crate::test_println!("  avg: {:.6}", _avg);
        }

        // Point count difference statistics
        if !point_count_diffs.is_empty() {
            point_count_diffs.sort();
            let _avg: f32 =
                point_count_diffs.iter().sum::<i32>() as f32 / point_count_diffs.len() as f32;
            crate::test_println!("\nPoint count difference (CUDA - Autoware):");
            crate::test_println!("  min: {}", point_count_diffs.first().unwrap());
            crate::test_println!(
                "  median: {}",
                point_count_diffs[point_count_diffs.len() / 2]
            );
            crate::test_println!("  max: {}", point_count_diffs.last().unwrap());
            crate::test_println!("  avg: {:.2}", _avg);
        }

        // Covariance ratio by point count
        crate::test_println!("\n--- Covariance Trace Ratio (CUDA/Autoware) by Point Count ---");
        let mut point_counts: Vec<usize> = cov_ratios_by_point_count.keys().copied().collect();
        point_counts.sort();

        let mut all_ratios: Vec<f32> = Vec::new();
        for pc in &point_counts {
            let ratios = cov_ratios_by_point_count.get(pc).unwrap();
            all_ratios.extend(ratios.iter());

            let avg: f32 = ratios.iter().sum::<f32>() / ratios.len() as f32;

            // Theoretical ratio if bug was (n-1)/n scaling
            let n = *pc as f32;
            let theoretical_wrong = (n - 1.0) * (n - 1.0) / (n * n);

            crate::test_println!(
                "  n={:3}: count={:4}, avg_ratio={:.4} (if (n-1)²/n² bug: {:.4})",
                pc,
                ratios.len(),
                avg,
                theoretical_wrong
        ...
        // Overall covariance ratio
        if !all_ratios.is_empty() {
            let overall_avg: f32 = all_ratios.iter().sum::<f32>() / all_ratios.len() as f32;
            all_ratios.sort_by(|a, b| a.partial_cmp(b).unwrap());

            crate::test_println!("\n--- Overall Covariance Ratio ---");
            crate::test_println!("  min: {:.4}", all_ratios.first().unwrap());
            crate::test_println!("  median: {:.4}", all_ratios[all_ratios.len() / 2]);
            crate::test_println!("  max: {:.4}", all_ratios.last().unwrap());
            crate::test_println!("  avg: {:.4}", overall_avg);
        ...
        crate::test_println!("\n=== END VOXEL DUMP COMPARISON ===\n");

        // Assert that we found enough matches
        assert!(
            matched > cuda_dump.voxels.len() / 2,
            "Should match at least 50% of voxels"
        );
    }
}
